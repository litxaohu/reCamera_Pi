...
def ai_output_tensor_parse(metadata: dict):
    ...
    global last_boxes, last_scores, last_keypoints, detect_count
    ...
    if np_outputs is not None:
        ...
        if scores is not None and len(scores) > 0:
            ...
            people_detected = np.sum(scores > args.detection_threshold)  # 比较每个得分
            if people_detected > 0:
                detect_count += people_detected  # 更新总检测人数
                current_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())  # 获取当前时间
                with open(log_file_path, "a") as log_file:
                    log_file.write(f"Time: {current_time}, Detected {people_detected} person(s)\n")
...
def ai_output_tensor_draw(request: CompletedRequest, boxes, scores, keypoints, stream='main'):
    ...
    with MappedArray(request, stream) as m:
        if boxes is not None and len(boxes) > 0:
            ...
            # # 在预览界面的右上角绘制检测到的人数
            # cv2.putText(m.array, f"Total Persons: {detect_count}", (m.array.shape[1] - 200, 30),
