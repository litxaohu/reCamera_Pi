...
from picamera2.devices.imx500.postprocess import COCODrawer  # 确保导入 COCODrawer
...
def ai_output_tensor_parse(metadata: dict):
    ...
    global last_boxes, last_scores, last_keypoints, detect_count, detected_persons
            ...
            for box, score in zip(boxes, scores):
                if score > args.detection_threshold:
                    ...
                    for detected_box in detected_persons:
                        if iou(box, detected_box) > 0.5:  # IoU大于0.5认为是同一个人
                            detected = True
                            break
                    ...
                    if not detected:
                        ...
                        with open(log_file_path, "a") as log_file:
                            log_file.write(f"Time: {current_time}, Detected person\n")
...
def ai_output_tensor_draw(request: CompletedRequest, boxes, scores, keypoints, stream='main'):
    ...
    with MappedArray(request, stream) as m:
        if boxes is not None and len(boxes) > 0:
            ...
            cv2.putText(m.array, f"Total Persons: {detect_count}", (10, 30),
