...
have_person = False  # 用于标记是否有人
new_count = 0  # 用于统计新检测到的人数
detect_count = 0  # 用于统计总人数
start_time = time.time()

# 保存已经检测到的人物的边界框，用于避免重复计数
detected_persons = []

# 记录日志的文件路径
log_file_path = "/home/seeed/detect.log"

# 创建一个 threading.Event 来同步主线程和 VLC 进程控制线程
detect_event = threading.Event()  # 默认未设置
...
def ai_output_tensor_parse(metadata: dict):
    global new_count, have_person
    """Parse the output tensor into a number of detected objects, scaled to the ISP output.""" 
    global last_boxes, last_scores, last_keypoints, detect_count, detected_persons, detect_event
    ...
    if np_outputs is not None:
        ...
        if scores is not None and len(scores) > 0:
            ...
            # 统计检测到的人数，避免重复计数
            scores = np.array(scores)  # 确保 scores 是一个 NumPy 数组
            people_detected = 0
            ...
            for box, score in zip(boxes, scores):
                if score > args.detection_threshold:
                    # 检查该框是否与已有的框重叠
                    detected = False
                    for detected_box in detected_persons:
                        if iou(box, detected_box) > 0.6:  # IoU大于0.5认为是同一个人
                            detected = True
                            break
                    
                    # 如果是新的检测到的人，增加计数并保存边界框
                    if not detected:
                        new_count += 1
                        detect_count += 1
                        detected_persons.append(box)  # 保存新检测到的人的边界框
        else:
            have_person = False
        ...
        with open(log_file_path, "a") as log_file:
            log_file.write(f"Time: {current_time}, Detected person: {have_person}, Total persons: {detect_count}\n")

        print(f"Have person: {have_person}")
        # print(f"Detected {new_count} new persons.")
        new_count = 0
...
def ai_output_tensor_draw(request: CompletedRequest, boxes, scores, keypoints, stream='main'):
    ...
    with MappedArray(request, stream) as m:
        if boxes is not None and len(boxes) > 0:
            ...
            # 在预览界面的左上角绘制检测到的人数，并调整字体大小
            cv2.putText(m.array, f"Total: {detect_count}", (10, 30),
